...
public class WikiLinkTag implements ParserTag {
	...
	private String buildInternalLinkUrl(ParserInput parserInput, int mode, String raw) {
		...
		try {
			WikiLink wikiLink = parseWikiLink(raw);
			parserInput.setTopicName(wikiLink.getArticle());
			...
			if (isImage(wikiLink)) {
				// parse as an image
				return parseImageLink(parserInput, mode, wikiLink);
			}
			if(isVideo(wikiLink)) {
				return parseVideoLink(parserInput, mode, wikiLink);
			}
			if(isAudio(wikiLink)) {
				return parseAudioLink(parserInput, mode, wikiLink);
			}
			...
			return LinkUtil.buildInternalLinkHtml(parserInput, context, virtualWiki, wikiLink, wikiLink.getText(), null, null, false);
		} catch (Exception e) {
	...
	private boolean isImage(WikiLink wikiLink) {
		return !wikiLink.getColon() && StringUtils.hasText(wikiLink.getNamespace()) && wikiLink.getNamespace().equals(NamespaceHandler.NAMESPACE_IMAGE);
	}
	
	private boolean isVideo(WikiLink wikiLink) {
		if(!wikiLink.getColon() && StringUtils.hasText(wikiLink.getNamespace()) && wikiLink.getNamespace().equalsIgnoreCase("Media")
				&& StringUtils.hasText(wikiLink.getDestination())) {
			String destination = wikiLink.getDestination().toLowerCase();
			return destination.endsWith(".mp4") || destination.endsWith(".m4v") || destination.endsWith(".mov");
		}
		return false;
	}
	
	private boolean isAudio(WikiLink wikiLink) {
		if(!wikiLink.getColon() && StringUtils.hasText(wikiLink.getNamespace()) && wikiLink.getNamespace().equalsIgnoreCase("Media")) {
			String destination = wikiLink.getDestination().toLowerCase();
			return destination.endsWith(".aac") || destination.endsWith(".mp3") || destination.endsWith("m4a");
		}
		return false;
	}
	...
	private String parseImageLink(ParserInput parserInput, int mode, WikiLink wikiLink) throws Exception {
		...
		return LinkUtil.buildImageLinkHtml(parserInput, context, virtualWiki, wikiLink.getDestination(), frame, thumb, align, caption, maxDimension, false, null, false);
	}
	
	private String parseVideoLink(ParserInput parserInput, int mode, WikiLink wikiLink) throws Exception {
		String context = parserInput.getContext();
		String virtualWiki = parserInput.getVirtualWiki();
		...
		if (StringUtils.hasText(wikiLink.getText())) {
			...
			while (tokens.hasMoreTokens()) {
				...
				if (token.equalsIgnoreCase("right")) {
					...
					Matcher m = IMAGE_SIZE_PATTERN.matcher(token);
					if (m.find()) {
						maxDimension = Integer.parseInt(m.group(1));
					} else {
						// FIXME - this is a hack.  images may contain piped links, so if
						// there was previous caption info append the new info.
						if (!StringUtils.hasText(caption)) {
		...
		return LinkUtil.buildVideoHtml(parserInput, context, virtualWiki, wikiLink.getDestination(), caption);
	}
	
	private String parseAudioLink(ParserInput parserInput, int mode, WikiLink wikiLink) throws Exception {
		String context = parserInput.getContext();
		String virtualWiki = parserInput.getVirtualWiki();
		return LinkUtil.buildAudioHtml(parserInput, context, virtualWiki, wikiLink.getDestination(), "");
	}
